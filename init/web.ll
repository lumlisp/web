...
(define (start-dev)
  ...
  (while #t
    (begin
      (system "ll web.ll serve > /tmp/llweb.log 2>&1 & echo $! > /tmp/llweb.pid")
      (usleep 200)
...
(cond
  ((null? args)
    (println "Usage: ll web.ll <command> [args]")
    (println "")
    (println "Commands:")
  ...
  ((string=? (car args) "migrate")
    (if (null? (cdr args))
      (println "Usage: ll web.ll migrate <up|down|status|rollback|reset|fresh> [n]")
      (begin
        (import "Migrations")
